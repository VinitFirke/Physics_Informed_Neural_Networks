...
def function_factory(lbfgs_model, train_x):
        ...
        for i, shape in enumerate(shapes):
            ...
            idx.append(tf.reshape(tf.range(count, count + sn_i, dtype=tf.int32), shape))
            part.extend([i] * sn_i)
            count += sn_i
        ...
        def f(params_1d):
            with tf.GradientTape() as tape:
                ...
                prediction = lbfgs_model(x)
                loss_value = prediction
            grads = tape.gradient(
                loss_value,
            ...
            if tf.math.mod(x=f.iter, y=1) == 0:
                tf.print(
                    ...
                    "| loss:",
                    tf.as_string(loss_value, scientific=True, precision=8),
                    "| grad:",
                    tf.as_string(
